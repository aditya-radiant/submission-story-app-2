package com.dicoding.picodiploma.submission_story_app.data.remotemediator
...
import com.dicoding.picodiploma.submission_story_app.data.local.RemoteKeys
...
class StoryRemoteMediator(
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, ListStoryItem>): RemoteKeys? {
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()?.let {
            database.remoteKeysDao().getRemoteKeysById(it.id)
        }
    }
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, ListStoryItem>): RemoteKeys? {
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.id?.let {
                database.remoteKeysDao().getRemoteKeysById(it)
            }
        }
    }
